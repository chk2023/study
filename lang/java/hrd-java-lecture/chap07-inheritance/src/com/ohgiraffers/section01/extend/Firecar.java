...
public class Firecar extends Car {
    ...
    * */
    ...
    /* @Override 어노테이션
    * JDK 1.5부터 추가된 문법으로 오버라이딩 성립 요건을 체크하여 성립되지 않은 경우 컴파일 에러를 발생시킨다.
    * 오버라이딩 하는 메소드는 기본적으로 부모 메소드 선언 내용을 그대로 작성해야 한다. */
    @Override
    public void soundHorn() {
        if (isRunning()) { //this.isRunning, super.isRunning 둘 다 사용 가능하다.
            /* runningStatus는 private 필드이므로 접근 불가능하며
            * 부모의 isRunning() 메소드를 통해서만 주행 중인 상태를 확인할 수 있으므로 메소드의 접근 제한자를 protected로 변경한다.
            * Car 클래스의 runningStatus를 protected로 변경하여 직접 접근할 수도 있지만 캡슐화의 원칙에 위배되므로 권장되지 않는다.
