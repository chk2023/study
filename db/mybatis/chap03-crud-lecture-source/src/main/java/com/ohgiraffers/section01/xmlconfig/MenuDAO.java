...
public class MenuDAO {
    ...
    public List<MenuDTO> selectAllMenu(SqlSession sqlSession) {
        ...
        * mapper의 namespace와 id를 통해서 구분한다. */
        return sqlSession.selectList("MenuMapper.selectAllMenu"); //여러 행 조회이기 때문에 selectList()
    }

    public MenuDTO selectMenuByMenuCode(SqlSession sqlSession, int menuCode) {
        /* 메서드의 두 번째 인자로 sql 구문을 수행할 때 필요한 값을 전달한다. */
        return sqlSession.selectOne("MenuMapper.selectMenuByMenuCode", menuCode); //한 행 조회이기 때문에 selectOne()
    }
