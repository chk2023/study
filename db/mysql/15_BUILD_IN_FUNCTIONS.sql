...
-- BIT_LENGTH(문자열) : 할당 된 비트 크기 반환, CHAR_LENGTH(문자열) : 문자열의 길이 반환, LENGTH(문자열) : 할당된 byte 크기 반환 
...
-- CONCAT(문자열1, 문자열2, ...) : 문자열 이어붙임, CONCAT_WS(구분자, 문자열1, 문자열2, ...) : 구분자와 함께 문자열을 이어붙임
...
-- FIND_IN_SET(찾을 문자열, 문자열 리스트) : 찾을 문자열의 위치 반환. (문자열 리스트는 , 기준으로 연결 되어 있음)
...
-- FORMAT(숫자, 소수점 자릿수): 1000 단위마다 콤마 표시를 해 주며 소수점 아래 자릿수(반올림)까지 표현한다.
...
-- TRIM : 양쪽 공백 제거
-- TRIM(방향 자를_문자열 FROM 문자열) : LEADING(앞), BOTH(양쪽), TRAILING(뒤)
SELECT TRIM(' MySQL '), TRIM(BOTH '@' FROM '@@@@MySQL@@@@');

-- REPEAT: 문자열을 횟수만큼 반복
SELECT REPEAT('재밌어', 30);
