...
-- (1) UNION
-- 두 개 이상의 select 문의 결과를 결합하여 중복 된 레코드를 제거한 후 반환하는 SQL 연산자
SELECT
	menu_code,
    menu_name,
    menu_price,
...
-- (2) UNION ALL
-- 두 개 이상의 select 문의 결과를 결합하여 중복 된 레코드를 제거하지 않고 모두 반환하는 SQL 연산자
SELECT
	menu_code,
    menu_name,
    menu_price,
...
-- (3) INTERSECT
-- 두 개 이상의 select 문의 결과 중 공통 되는 레코드만을 반환하는 연산자
...
-- MySQL은 MINUS 연산자를 제공하지 않는다. 단, LEFT JOIN을 활용해서 구현 가능하다.    
SELECT
	a.menu_code,
    a.menu_name,
    a.menu_price,
