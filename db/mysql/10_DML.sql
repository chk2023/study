...
-- null 허용 가능 컬럼 또는 auto_increment 설정 된 컬럼을 제외하고 insert할 컬럼을 지정해서 insert가능alter
INSERT INTO
...
-- 2. UPDATE : 테이블에 기록된 컬럼의 값을 수정하는 구문으로 행 갯수의 변화는 없다.
SELECT
	menu_code,
    category_code
FROM
...
-- MySQL에서는 update, delete 시 자기 자신 테이블의 데이터를 사용하면 1093 에러가 발생한다. 
-- 이 때 SUBQUERY를 한 번 더 사용하여 임시 테이블로 사용하면 해결할 수 있다.
UPDATE
	tbl_menu
SET
	category_code = 6
