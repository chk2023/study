...
-- 컬럼 별칭
-- AS '별칭'의 형태로 작성하나 별칭에 특수문자가 없다면 생략하고 작성 가능하다.
SELECT
	menu_code AS 'code',
    menu_name AS name,
    menu_price price
FROM
	tbl_menu
ORDER BY
	price; -- 컬럼 별칭 ORDER BY절 사용 가능

-- 테이블 별칭
-- 어떤 테이블 소속인지 쉽게 알 수 있게 하며 AS를 써도 되고 생략도 가능하다.
SELECT 
	a.category_code,
    a.menu_name
FROM
...
-- 1-1. ON 을 이용한 문법 (연결에 사용 될 컬럼명이 동일하지 않거나, 동일하거나 모두 사용 가능)
SELECT
	a.menu_name,
    b.category_name
FROM
...
-- (3) RIGHT JOIN : 두 번째(오른쪽) 테이블의 모든 레코드와 첫 번째(왼쪽) 테이블에서 일치하는 레코드를 반환하는 SQL JOIN 유형
SELECT
    a.menu_name,
    b.category_code
FROM
