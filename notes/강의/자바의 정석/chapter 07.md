...
하지만 Fighter 클래스의 멤버 모두를 사용할 수는 없고 인터페이스 Fightable의 메소드만 사용 가능하다. 대입받은 참조변수가 리모콘이라고 생각하면 리모콘에 버튼이 두 개이기 때문에 아무리 객체의 멤버가 많아도 버튼 두 개에 해당하는 객체만 사용가능 한 것이다.
...
![[Pasted image 20231019133015.png]]
껍데기(선언)와 알맹이(구현)를 분리시킬 수 있다.
이렇게 해야 변경에 유리하다. 

![[Pasted image 20231019133347.png]]
클래스 A가 클래스 B를 사용한다. (스프링에선 의존한다라는 말을 많이 사용)
만약 클래스 A가 클래스 C를 써야한다면 A는 코드를 변경해야 한다.
하지만 클래스 A가 인터페이스를 구현한다면 인터페이스는 껍데기에 불과하기 때문에 안에 알맹이가 클래스 C로 바뀐다고 해도 클래스 A는 코드가 변경되는 것이 없다.

### 인터페이스의 장점 2

![[Pasted image 20231019134709.png]]
...
![[Pasted image 20231019222300.png]]
인터페이스에 새로운 메서드를 추가하면 이는 추상 메서드이기 때문에 기존에 이 인터페이스를 구현한 모든 클래스가 이 메서드를 구현해야 한다.
이 문제를 해결하기 위해 구현부가 있는 메서드를 추가해준다. 이것이 default method.

하지만 디폴트 메서드를 가지면 다시 충돌 문제가 생긴다. 
인터페이스를 구현한 클래스에서 디폴트 메서드를 오버라이딩해야 한다.
디폴트 메서드와 조상 클래스의 메서드가 충돌하면 조상 클래스의 메서드가 상속된다. 
복잡하게 생각하지 말고 직접 오버라이딩하면 된다. 
