...
<body>
    ...
    <h3>우선 순위 테스트</h3>
    <!-- 대상 scope를 생랴할 경우 좁은 범위부터 값을 탐색한다.
    requestScope가 sessionScope보다 범위가 좁기 때문에 requestScope가 우선적으로 탐색된다.
    만약 member라는 값이 requestScope에 없었을 경우에는 sessionScope의 값이 출력 된다. -->
    이름 : ${ member.name } <br>
    나이 : ${ member.age } <br>
